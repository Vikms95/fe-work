...
var ThreeBSP = function () {
  function ThreeBSP(geometry) {
    ...
    if (geometry.isBufferGeometry) {
      ...
      var index = void 0;
      if (geometry.index) index = geometry.index.array;else {
        index = new Array(posattr.array.length / posattr.itemSize | 0);
        for (var _i = 0; _i < index.length; _i++) {
          index[_i] = _i;
        }
      }
      var mod = index.length % 3;

      var _loop = function _loop(l, _i2) {
        polygon = new Polygon();
        /*
            face [i .. i+2]
            group [index, count]
             RateServices.StartDate<=Fin AND (RateServices.EndDate IS NULL OR RateServices.EndDate>=Ini)
            index<=i+2 && (index+count-1>=i)
             face [2,4]
            group [0,1]
            no 
            face [2,4]
            ...
            no
         */
        var idx = geometry.groups.findIndex(function (g) {
          return g.start <= _i2 + 2 && g.start + g.count - 1 >= _i2;
        });

        if (idx >= 0) polygon.setMateralIndex(geometry.groups[idx].materialIndex);
        ...
        for (var j = 0; j < 3; j++) {
          ...
          var normal = new THREE.Vector3(normalattr.array[vp], normalattr.array[vp + 1], normalattr.array[vp + 2]);
          uvs = new THREE.Vector2(uvattr.array[vt], uvattr.array[vt + 1]);

          vertex = new Vertex(x, y, z, normal, uvs);
      ...
      for (var _i2 = 0, l = index.length - mod; _i2 < l; _i2 += 3) {
        _loop(l, _i2);
      }
    } else {
  ...
  _createClass(ThreeBSP, [{
    ...
    value: function toBufferGeometry() {
      var i,
          j,
          matrix = new THREE.Matrix4().invert(this.matrix),
      ...
      for (i = 0; i < polygon_count; i++) {
        ...
        for (j = 2; j < polygon_vertice_count; j++) {
          add(polygon.vertices[0], polygon.normal, polygon.materialIndex);
          add(polygon.vertices[j - 1], polygon.normal, polygon.materialIndex);
          add(polygon.vertices[j], polygon.normal, polygon.materialIndex);
        }
      }
      ...
      while (i < nindices) {
        var _j = i;
        var materialIndex = materialsIndex[i++];
        var count = 1;

        for (i; i < nindices; i++) {
          if (materialIndex == materialsIndex[i]) count++;else break;
        }

        geometry.addGroup(_j, count, materialIndex);
      }

      return geometry;

      function add(v, n, materialIndex) {
        ...
        for (k = k2 = k3 = 0; k < nvertexs; k++, k2 += 3, k3 += 2) {

          if (positions[k2] == v.x && positions[k2 + 1] == v.y && positions[k2 + 2] == v.z && normals[k2] == n.x && normals[k2 + 1] == n.y && normals[k2 + 2] == n.z && uvs[k3] == uv.x && uvs[k3 + 1] == uv.y && materialsIndex[k] == materialIndex) {
            materialsIndex[nindices] = materialIndex;
            indices[nindices++] = k;
...
var Polygon = function () {
  ...
  _createClass(Polygon, [{
    key: 'setMateralIndex',
    value: function setMateralIndex(i) {
    ...
    value: function clone() {
      ...
      polygon.setMateralIndex(this.materialIndex);
    ...
    value: function splitPolygon(polygon, coplanar_front, coplanar_back, front, back) {
      ...
      if (classification === COPLANAR) {
        ...
        if (f.length >= 3) front.push(new Polygon(f).calculateProperties().setMateralIndex(polygon.materialIndex));
        if (b.length >= 3) back.push(new Polygon(b).calculateProperties().setMateralIndex(polygon.materialIndex));
      }
    }
  }]);
