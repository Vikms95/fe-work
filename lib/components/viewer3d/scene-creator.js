...
function parseData(state, sceneData, actions, catalog) {
  ...
  planData.grid = (0, _gridCreator2.default)(state, sceneData);
...
function addHole(sceneData, planData, layer, holeID, catalog, holesActions) {
  ...
  return catalog.getElement(holeData.type).render3D(holeData, layer, sceneData).then(function (object) {
    ...
    var vertex0 = layer.vertices.get(line.vertices.get(!v2First ? 0 : 1));
    var vertex1 = layer.vertices.get(line.vertices.get(!v2First ? 1 : 0));
    var thickness = line.properties.getIn(['thickness', 'length']);

    var _Line$createVertexAnd = _export.Line.createVertexAndVectorsB(vertex0.x, vertex0.y, vertex1.x, vertex1.y, v2First, thickness),
        vu = _Line$createVertexAnd.vu;

    var offset = !v2First ? holeData.offset : 1 - holeData.offset;
    ...
    */
    ...
    //todo Posicionamiento objeto del hole
    // Añado 5 para ajustarlo al agujero de momento
    pivot.position.x = vertex0.x + distance * offset * Math.cos(alpha);
    pivot.position.y = holeAltitude + layer.altitude;
    // Resto 15 para ajustarlo al agujero de momento
...
var isChildMaterialNestedArray = function isChildMaterialNestedArray(child) {
  return child.material.materials && typeof child.material.materials.length !== 'undefined';
};
...
function applyOpacity(object, opacity) {
  object.traverse(function (child) {
    ...
    if (child instanceof Three.Mesh) {
      if (isChildMaterialNestedArray(child)) {
