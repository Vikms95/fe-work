...
var Hole = function () {
  ...
  _createClass(Hole, null, [{
    ...
    value: function create(state, layerID, type, lineID, offset, properties) {
      ...
      var info = state.catalog.getIn(['elements', type, 'info']);
      var options = {
        id: holeID,
        name: _export2.NameGenerator.generateName('items', info.get('title')),
      ...
      var hole = state.catalog.factoryElement(type, options, properties);
    ...
    key: 'calcOldHole',
    value: function calcOldHole(state, layerID, holeID) {
      var hole = state.getIn(['scene', 'layers', layerID, 'holes', holeID]);
      var width = hole.properties && hole.properties.has('width') ? hole.properties.get('width').get('length') : state.catalog.factoryElement(hole.type).properties.getIn(['width', 'length']);
      var half_width = width / 2;
      var lineID = hole.line;
      var offset = hole.offset;
      var line = state.getIn(['scene', 'layers', layerID, 'lines', lineID]);
      var vertices = state.getIn(['scene', 'layers', layerID, 'vertices']);
    ...
    key: 'calcOldHolesListByVertexID',
    value: function calcOldHolesListByVertexID(state, layerID, vertexID, oldHoles) {
      var vertex = state.getIn(['scene', 'layers', layerID, 'vertices', vertexID]);

      vertex.lines.forEach(function (lineID) {
        var line = state.getIn(['scene', 'layers', layerID, 'lines', lineID]);

        if (line.holes) line.holes.forEach(function (holeID) {
    ...
    key: 'calcOldHolesByVertexID',
    value: function calcOldHolesByVertexID(state, layerID, vertexID) {
      var oldHoles = [];
    ...
    key: 'calcOldHolesByLineID',
    value: function calcOldHolesByLineID(state, layerID, lineID) {
      var oldHoles = [];
      var vertices = state.getIn(['scene', 'layers', layerID, 'lines', lineID, 'vertices']);

      if (vertices) vertices.forEach(function (vertexID) {
        oldHoles = Hole.calcOldHolesListByVertexID(state, layerID, vertexID, oldHoles);
      });
    ...
    key: 'calcNewHolesFromOldHoles',
    value: function calcNewHolesFromOldHoles(state, layerID, oldHoles) {
      var newHoles = [];
      var valid = true;

      oldHoles.forEach(function (oldHole) {
        var oldHoleNew = Hole.calcOldHole(state, layerID, oldHole.holeID);
        var invN = oldHole.lineLength / oldHoleNew.lineLength;
        var offsetNew = invN * oldHole.offset;
        var offsetWidthNew = offsetNew + invN * oldHole.offsetWidth;
        var newHole = { holeID: oldHole.holeID, offset: offsetNew, offsetWidth: offsetWidthNew };

        //console.log(`holeID: ${newHole.holeID}, offset: ${newHole.offset}, offsetWidth: ${newHole.offsetWidth}`);

        if (offsetNew > 1 || offsetWidthNew > 1) valid = false;
    ...
    key: 'updateHolesFromNewHoles',
    value: function updateHolesFromNewHoles(state, layerID, newHoles) {
      newHoles.newHoles.forEach(function (newHole) {
        var holeID = newHole.holeID;
        var hole = state.getIn(['scene', 'layers', layerID, 'holes', holeID]);
