...
var _export3 = require('../proxies/export');

var _export4 = require('../actions/export');
...
var Project = function () {
  ...
  _createClass(Project, null, [{
    ...
    value: function SetUserAuthenticated(state, value) {
    ...
    key: 'setPreference',
    value: function setPreference(state, prefs, prefsInfo) {
      var _prefs = new _immutable.Map(prefs);
      var unit = _prefs.get(_constants.UNIDADMEDIDA);

      if (prefsInfo) _export3.Prefs.setPrefsInfo(prefsInfo);
      state = state.setIn(['prefs'], new _immutable.Map(prefs));
      state = state.set('prefs', _prefs);
      state = state.setIn(['scene', 'prefs'], _prefs);
      if (unit && state.scene.unit != unit) state = state.setIn(['scene', 'unit'], unit);

      return { updatedState: state };
    }
  }, {
    key: 'updatePreference',
    value: function updatePreference(state, nombre, valor) {
      /*
      ...
      */
      ...
      return {
        updatedState: function updatedState(dispatch) {
          var proxy = new _export3.Prefs.PrefsProxy();
          var p = {
            nombre: nombre,
            valor: valor
          };

          proxy.updatePreference(p).then(function (data) {
            return dispatch(_export4.projectActions.updatePreferenceCompleted(p.nombre, p.valor));
          }).catch(function (error) {
            return dispatch(_export4.projectActions.updatePreferenceFailed(p.nombre, p.valor, error));
          });
    ...
    key: 'updatePreferenceCompleted',
    value: function updatePreferenceCompleted(state, nombre, value) {
      state = state.setIn(['prefs', nombre], value);

      var _prefs = state.get('prefs');
      //let prefs = state.getIn('scene', 'prefs');
      state = state.setIn(['scene', 'prefs'], _prefs);
      if (name == _constants.UNIDADMEDIDA) state = state.setIn(['scene', 'unit'], value);

      console.log("completed");
      return { updatedState: state };
    }
  }, {
    key: 'updatePreferenceFailed',
    value: function updatePreferenceFailed(state, nombre, value, error) {
      alert(error.detail || error.statusText || error.status || error);
    ...
    value: function next_Drawing_Item(state, layerID) {
      state = _export2.Layer.next_Drawing_ItemOnSelected(state, layerID).updatedState;
    ...
    value: function remove(state) {
      ...
      state = _export2.Layer.detectAndUpdatePerimeters(state, selectedLayer).updatedState;
      state = _export2.Perimeter.reCalcAllVertexsB(state, selectedLayer).updatedState;
      state = this.setIsElementSelected(state).updatedState;
    ...
    key: 'setIsElementSelected',
    value: function setIsElementSelected(state) {
      var selectedElements = (0, _selectors.getSelectedElementsToJS)(state);
      var isElementSelected = selectedElements.some(function (key) {
        return typeof key[1][0] !== 'undefined';
      });
