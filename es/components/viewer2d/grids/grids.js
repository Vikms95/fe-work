  ...
  var renderedGrids = grids.entrySeq().map(function (_ref2) {
    ...
    switch (grid.type) {
      case 'horizontal-streak':
        return React.createElement(GridHorizontalStreak, { key: gridID, state: state, width: width, height: height, grid: grid });

      case 'vertical-streak':
        return React.createElement(GridVerticalStreak, { key: gridID, state: state, width: width, height: height, grid: grid });
