  ...
  if (geom.isGeometry) {
    ...
    for (var _i2 = 0, pli = 0, l = index.length; _i2 < l; _i2 += 3, pli++) {
      ...
      for (var _j = 0; _j < 3; _j++) {
        ...
        _vertices[_j] = new Vertex({ x: x, y: y, z: z }, { x: nx, y: ny, z: nz }, uvAttr && { x: uvAttr.array[vt], y: uvAttr.array[vt + 1], z: 0 }, colorAttr && { x: colorAttr.array[vt], y: colorAttr.array[vt + 1], z: colorAttr.array[vt + 2] });
      }

      polygons[pli] = new Polygon(_vertices, objectIndex);
    }
...
CSG.toMesh = function (csg, toMatrix, toMaterial) {
  var geometry = CSG.toGeometry(csg);
