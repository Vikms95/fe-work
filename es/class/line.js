...
import { Group, Layer, Hole, Vertex, Perimeter, Project } from './export';

import { IDBroker, NameGenerator, GeometryUtils, SnapUtils, SnapSceneUtils } from '../utils/export';

import { MODE_IDLE, MODE_WAITING_DRAWING_LINE, MODE_DRAWING_LINE, MODE_DRAGGING_LINE, _2_PI, PI_2, _3_PI_2, ALTOPARED, FONDOPARED } from '../constants';

import { getCacheAlto, getCacheFondo, getPrefsAlto, getPrefsFondo, setCacheAlto, setCacheFondo, setCacheAngulo, isMultipleSelection } from '../selectors/selectors';
...
var Line = function () {
  ...
  _createClass(Line, null, [{
    ...
    value: function isLinesJoin(state, layerID, line1ID, line2ID) {
      ...
      if (vertex11.id == vertex20.id || GeometryUtils.samePoints(vertex11, vertex20)) return 1;
      ...
      if (vertex21.id == vertex10.id || GeometryUtils.samePoints(vertex21, vertex10)) return -1;
    ...
    value: function getAngleV0_pcl(layer, line) {
      ...
      var u = GeometryUtils.diffVector(v10, v11);
      var angle_u = GeometryUtils.fixAngleRadNeg(PI_2 - GeometryUtils.angleVector(u));
    ...
    value: function getAngleRadV0ByAngle_pcl(layer, line, angleDegree) {
      ...
      if (!lineIDV0) return PI_2 - angleRad;
      ...
      var v10 = vertices.get(v10ID);
      var v11 = vertices.get(v11ID);
      var u1 = GeometryUtils.diffVector(v11, v10);
      var angle_u1 = GeometryUtils.angleVector(u1);
      var startAngle = GeometryUtils.fixAngleRadNeg(angle_u1);
    ...
    value: function getAngleAndArcL11L20_pcl(layer, line, dist) {
      ...
      var u1 = GeometryUtils.diffVector(v11, v10);
      var u2 = GeometryUtils.diffVector(v20, v21);
      var angle_u1 = GeometryUtils.angleVector(u1);
      ...
      var startAngle = GeometryUtils.fixAngleRadNeg(angle_u1 + PI_2);
      var endAngle = GeometryUtils.fixAngleRadNeg(GeometryUtils.angleVector(u2) + PI_2);
      //let endAngle = GeometryUtils.fixAngleRadNeg(startAngle + angle)
      var angle = startAngle <= endAngle ? endAngle - startAngle : _2_PI - startAngle + endAngle;
    ...
    value: function createVertexAndVectorsB(x0, y0, x1, y1, v2First, thickness) {
      ...
      var v = GeometryUtils.diffVector(v0, v1);

      if (v.x == 0 && v.y == 0) v.x = 0.1;

      var vu = GeometryUtils.normalizeVector(v);
      var u = GeometryUtils.orthoVector(vu);
      var unew = GeometryUtils.multScalarVector(thickness, u);
      var v2 = GeometryUtils.addVector(v0, unew);
      var v3 = GeometryUtils.addVector(v1, unew);
    ...
    value: function create(state, layerID, type, x0, y0, x1, y1, properties, argsEx) {
      var p = fromJS(properties || {});
      var alto = getCacheAlto(state) || getPrefsAlto(state);
      var fondo = getCacheFondo(state) || getPrefsFondo(state);
      ...
      var _Vertex$add3 = Vertex.add(stateV1, layerID, vv2.x, vv2.y, 'lines', lineID),
          stateV2 = _Vertex$add3.updatedState,
          v2 = _Vertex$add3.vertex;

      var _Vertex$add4 = Vertex.add(stateV2, layerID, vv3.x, vv3.y, 'lines', lineID),
    ...
    value: function remove(state, layerID, lineID) {
      ...
      if (line) {
        ...
        if (line.perimeter != null) state = Perimeter.removeElement(state, layerID, line.perimeter, 'lines', lineID).updatedState;
    ...
    value: function addFromPoints(state, layerID, type, points, properties, holes) {
      ...
      pointsPair.forEach(function (_ref5) {
        ...
        var v2First = !GeometryUtils.samePoints(v0, { x: x1, y: y1 });
    ...
    value: function replaceVertex(state, layerID, lineID, vertexIndex, x, y) {
      ...
      var _Vertex$add5 = Vertex.add(state, layerID, x, y, 'lines', lineID),
    ...
    value: function beginDraggingLine(state, layerID, lineID, x, y) {
      ...
      var oldHoles = Hole.calcOldHolesByLineID(state, layerID, lineID);
    ...
    value: function modifyCoordsOnKeyDown(x1, x2, y1, y2, keyCode) {
      var value = GeometryUtils.pointsDistance(x1, y1, x2, y2);
    ...
    value: function setAttributes(state, layerID, lineID, lineAttributes) {
      ...
      state = state.mergeIn(['scene', 'layers', layerID, 'lines', lineID], fromJS(lAttr))
      ...
      if (v0id == v1id) {
        var _Vertex$add6 = Vertex.add(state, layerID, x1new, y1new, 'lines', lineID),
