...
import { MODE_VIEWING_CATALOG, MODE_CONFIGURING_PROJECT, MODE_IDLE, UNIDADMEDIDA } from '../constants';
import { State, Catalog } from '../models';
import { history } from '../utils/export';
import { Layer, Group, Line, Hole, Perimeter, Item, HorizontalGuide, VerticalGuide } from '../class/export';
...
var Project = function () {
  ...
  _createClass(Project, null, [{
    ...
    value: function setPreference(state, prefs, prefsInfo) {
      var _prefs = new Map(prefs);
      var unit = _prefs.get(UNIDADMEDIDA);

      if (prefsInfo) Prefs.setPrefsInfo(prefsInfo);
      ...
      */
      ...
      return {
        updatedState: function updatedState(dispatch) {
          var proxy = new Prefs.PrefsProxy();
          var p = {
            nombre: nombre,
            valor: valor
          };
          ...
          proxy.updatePreference(p).then(function (data) {
            return dispatch(projectActions.updatePreferenceCompleted(p.nombre, p.valor));
          }).catch(function (error) {
            return dispatch(projectActions.updatePreferenceFailed(p.nombre, p.valor, error));
          });
    ...
    value: function updatePreferenceCompleted(state, nombre, value) {
      ...
      state = state.setIn(['scene', 'prefs'], _prefs);
      if (name == UNIDADMEDIDA) state = state.setIn(['scene', 'unit'], value);
    ...
    value: function setIsElementSelected(state) {
      var selectedElements = getSelectedElementsToJS(state);
