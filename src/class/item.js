...
class Item {
  ...
  static create ( state, layerID, type, x, y, rotation ) {
    ...
    let item = state.catalog.factoryElement( type, options );
  ...
  static updateDrawingItem ( state, layerID, x, y ) {
    if ( state.hasIn( [ 'drawingSupport', 'currentID' ] ) ) {
      state = state.updateIn( [ 'scene', 'layers', layerID, 'items', state.getIn( [ 'drawingSupport', 'currentID' ] ) ], item => item.merge( {
        x,
        y
      } ) );
    } else {
