...
export default function MenuPreferencias ( props ) {
  ...
  const onChange = ( name, value ) => {
    const newPrefsState = state.prefsState;
    newPrefsState[ name ] = value;

    setState( ( prevState ) => (
      { ...prevState, prefsState: newPrefsState }
    ) );
            ...
            if ( pref.typeValor === 'Texto' ) {
              return (
                <div key={ i }>
                  <div style={ { display: 'flex', flexDirection: 'row', alignItems: 'center' } }>
                    ...
                    {/* TODO the only thing that changes is this snippet  */ }

                    <input style={ STYLE_INPUT } type={ 'text' }
                      onChange={ ( e ) => onChange( pref.nombre, e.target.value ) }
                ...
                <div key={ i }>
                  <div style={ { display: 'flex', flexDirection: 'row', alignItems: 'center' } }>
                    ...
                    <div style={ { display: 'flex' } }>
                      <input style={ { ...STYLE_INPUT, width: '2em', margin: '0' } } type={ 'checkbox' }
                        onChange={ ( e ) => onChange( pref.nombre, e.target.checked ) }
              ...
              return (
                <div key={ i }>
                  <div style={ { display: 'flex', flexDirection: 'row', alignItems: 'center' } }>
                    <label style={ { ...BASE_STYLE_LABEL, margin: '20px', width: '7em' } }>{ pref.nombre }</label>
                    <input style={ STYLE_INPUT } type={ 'number' }
                      onChange={ ( e ) => onChange( pref.nombre, e.target.value ) }
                      value={ state.prefsState[ pref.nombre ] }
                      name={ pref.nombre }
                    />
