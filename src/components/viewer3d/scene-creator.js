...
function addHole ( sceneData, planData, layer, holeID, catalog, holesActions ) {
  ...
  return catalog.getElement( holeData.type ).render3D( holeData, layer, sceneData ).then( object => {
    ...
    //let holeThickness = holeData.properties.getIn(['thickness', 'length']);
    let depth = Hole.getDepth( holeData, layer );
    ...
    */
    ...
    pivot.position.z = -vertex0.y - distance * offset * Math.sin( alpha ) - depth * Math.cos( alpha );
...
const isChildMaterialNestedArray = ( child ) => (
  child.material.materials && typeof child.material.materials.length !== 'undefined'
);
