...
export default function Grids({ state, scene }) {
  ...
  let renderedGrids = grids.entrySeq().map(([gridID, grid]) => {
    switch (grid.type) {
      case 'horizontal-streak':
        return (<GridHorizontalStreak key={gridID} state={state} width={width} height={height} grid={grid} />);

      case 'vertical-streak':
        return (<GridVerticalStreak key={gridID} state={state} width={width} height={height} grid={grid} />);
